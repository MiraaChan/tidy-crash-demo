...
def _run_tidy(ctx, exe, flags, compilation_context, infile, discriminator):
    ...
    args.add_all(extra_header, before_each = "-isystem")
...
def _rule_sources(ctx):
    ...
    if hasattr(ctx.rule.attr, "srcs"):
        for src in ctx.rule.attr.srcs:
            srcs += [item for item in src.files.to_list() if item.is_source]
    return srcs
...
clang_tidy_aspect = aspect(
    ...
    attrs = {
        ...
        "_clang_tidy": attr.label(default = Label("//third_party/clang_tidy:clang_tidy")),
    },
